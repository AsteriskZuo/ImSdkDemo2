...
@property (nonatomic, strong) DUIRecordPopView *record;
            ...
            if (!granted) {
                ...
                [ac addAction:[UIAlertAction actionWithTitle:@"去开启" style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
                    ...
                    if ([app canOpenURL:settingsURL]) {
                        UIApplicationOpenExternalURLOptionsKey ss = UIApplicationOpenURLOptionUniversalLinksOnly;
                        NSDictionary<UIApplicationOpenExternalURLOptionsKey,id>* dict = [NSDictionary dictionaryWithObject:@1 forKey:ss];
                        [app openURL:settingsURL options:dict completionHandler:nil];
//                        [app openURL:settingsURL];
    ...
    if(permission == AVAudioSessionRecordPermissionGranted){
        if(!_record){
            _record = [[DUIRecordPopView alloc] init];
...
- (void)startRecord
{
    ...
    NSString *path = [TUIKit_Voice_Path stringByAppendingString:[DHelper genVoiceName:nil withExtension:@"m4a"]];
