...
//  ImSdkDemo
//
//  Created by AsteriskZuo on 2020/2/16.
...
- (void)setupViews
{
    ...
    [_faceCollectionView registerClass:[DUIFaceCell class] forCellWithReuseIdentifier:TFaceCell_ReuseId];
...
- (void)setData:(NSArray<DUIFaceGroup*> *)data
{
    ...
    for (NSInteger groupIndex = 0; groupIndex < _faceGroups.count; ++groupIndex) {
        DUIFaceGroup *group = _faceGroups[groupIndex];
    ...
    for (NSInteger curSection = 0; curSection < _sectionCount; ++curSection) {
        ...
        DUIFaceGroup *face = _faceGroups[groupIndex.integerValue];
...
{
    DUIFaceCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:TFaceCell_ReuseId forIndexPath:indexPath];
    ...
    if(indexPath.row == itemCount - 1 && group.needBackDelete){
        ...
        if(index.integerValue < group.faces.count){
            DUIFaceCellData *data = group.faces[index.integerValue];
            [cell setData:data];
        }
        else{
...
{
    int groupIndex = [_groupIndexInSection[indexPath.section] intValue];
    DUIFaceGroup *faces = _faceGroups[groupIndex];
